...
Mobile-first tournament software for a private annual golf trip. The current milestone provides the database foundation, Axum REST API, deterministic development data, read-only React views, atomic round opening, and audited individual/scramble scorecard APIs.
...
Authentication and authorization, the mobile scoring UI, round completion/locking, live leaderboards, course administration, and admin forms are intentionally deferred to later milestone 2 or 3 slices. The user/role, score, audit, locking, and handicap snapshot schema is already present so those features can be added without remodelling the core data.
