...
const samples = trace.samples.map(sample => ({ route: sample.route, width: sample.width, repeat: sample.repeat,
  phases: ['initial', ...sample.phases.map(p => p.name)].map(name => {
    ...
    return { name, list: counts(requests.filter(r => r.path.endsWith('/matches'))), table: counts(requests.filter(r => r.path.endsWith('/match-table'))),
  ...
  }), held: sample.server.filter(r => r.held).map(r => ({ path: r.path, phase: r.phase, aborted: r.aborted, closedBeforeRelease: r.closeMs < r.releaseMs, finished: r.finishMs !== undefined })),
...
if (!samples.length || samples.some(s => s.errors.length || s.serverErrors.length || s.overflow || s.pendingNonSse || s.finalEpoch !== 7 || s.held.some(r => !r.aborted || !r.closedBeforeRelease || r.finished))) throw new Error('Incomplete/failed sample')
...
const header = 'route,width,repeat,phase,listStarts,listCompleted,listAborted,tableStarts,tableCompleted,tableAborted,roundStarts,authStarts,streamStarts'
writeFileSync(resolve(out, 'samples.csv'), header + '\n' + samples.flatMap(s => s.phases.map(p => [s.route,s.width,s.repeat,p.name,p.list.starts,p.list.completed,p.list.aborted,p.table.starts,p.table.completed,p.table.aborted,p.rounds.starts,p.auth.starts,p.streamStarts].join(','))).join('\n') + '\n')
