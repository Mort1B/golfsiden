...
export function installObserver(origin) {
  ...
  new MutationObserver(() => {
    ...
    const listNodes = [...document.querySelectorAll('.match-list')].map(node => {
      if (!nodes.has(node)) nodes.set(node, ++nodeSerial)
      return nodes.get(node)
    })
    const value = { cards: cards.length, rows: rows.length, epochs, listNodes, denied: document.body?.textContent.includes('Synthetic access denied') ?? false }
