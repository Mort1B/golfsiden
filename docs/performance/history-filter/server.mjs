...
export async function startFixture(root, probe) {
  ...
  const server = createServer((req, res) => {
    const state = current, url = new URL(req.url, 'http://localhost'), path = url.pathname
    ...
    let body, cards, type = 'application/json'
    if (path === '/probe.js') { body = probe; type = 'text/javascript' }
      ...
      else if (/^\/api\/rounds\/[^/]+\/match-play\/matches$/.test(path)) {
        value = state.data.listings.get(path.split('/')[3])
        const player = url.searchParams.get('player_id')
        if (player) {
          const matches = value.matches.filter(m => m.opponents.some(p => p.player_id === player))
          value = { ...value, player_id: player, matches, writable_match_ids: value.writable_match_ids.filter(id => matches.some(m => m.match_id === id)) }
        }
      }
      else { state.errors.push(`Unexpected ${path}`); res.writeHead(404).end(); return }
      if (path.endsWith('/matches')) cards = value.matches.length
    ...
    const row = { path, query: url.search, cards, phase: state.phase, epoch: state.epoch, rawBytes: bytes.length, gzipBytes: encoded.length }
