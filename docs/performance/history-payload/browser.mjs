...
const bundled = await build({ entryPoints: [resolve(root, 'docs/performance/history-payload/probe.mjs')], bundle: true, minify: true, write: false, format: 'iife', platform: 'browser' })
...
try {
  for (const scenario of selected) {
    const data = dataFor(scenario), full = [...data.listings.values()], subset = full.map(playerSubset)
    ...
    const measurements = await probePage.evaluate(({ full, subset, playerId }) => {
      ...
      const run = (name, lists, batch) => {
        ...
        for (let i = 0; i < iterations; i++) decoded = parsed.map(l => window.decodeListingProbe(l, l.round_id))
      ...
      const narrowed = subset.map(l => window.decodeListingProbe(l, l.round_id))
    ...
    for (const width of scenario.widths) for (let repeat = 0; repeat < Number(process.env.HISTORY_REPEATS ?? 3); repeat++) {
      ...
      for (const route of order) {
        ...
        const metrics = await page.evaluate(mark => {
          const resources = performance.getEntriesByType('resource').filter(r => r.startTime >= mark && /\/api\/.*(matches|match-table)$/.test(new URL(r.name).pathname)).map(r => ({ path: new URL(r.name).pathname, startTime: r.startTime, responseStart: r.responseStart, responseEnd: r.responseEnd, transferSize: r.transferSize, encodedBodySize: r.encodedBodySize, decodedBodySize: r.decodedBodySize }))
        ...
        for (const row of [...refresh, ...tables]) {
          const resource = metrics.resources.find(r => r.path === row.path)
  ...
  writeFileSync(resolve(out, 'evidence.json'), JSON.stringify({ measuredAt: new Date().toISOString(), sourceCommit: execFileSync('git', ['rev-parse', 'HEAD'], { cwd: root, encoding: 'utf8' }).trim(), sourceStatus: execFileSync('git', ['status', '--short', '--', 'frontend', 'backend'], { cwd: root, encoding: 'utf8' }).trim(), browser: version, node: process.version,
    conditions: { latencyMs: 100, downloadBytesPerSecond: 200000, uploadBytesPerSecond: 93750, cpuSlowdown: 4, repeats: Number(process.env.HISTORY_REPEATS ?? 3), api: 'native synthetic gzip HTTP and SSE; no backend/database', timing: 'match-event refresh after settled initial and open authority passes; isolated warmed decoder batches separately; no production source modification' }, probeHash: hash(probeCode),
