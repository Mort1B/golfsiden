...
if (evidence.samples.length !== 54 || evidence.sourceStatus || evidence.probes.some(p => !p.parity) || evidence.samples.some(s => s.errors.length || s.serverErrors.length || s.metrics.overflow)) throw new Error('Incomplete/dirty/failed final evidence')
