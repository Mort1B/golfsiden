...
for run in range(1, 4):
    for size in [12, 24, 100]:
        ...
        for sample in data['samples']:
            ...
            role = sample['role']
            attempts = 2*k if role in ['admin', 'scorer'] else k+1 if role == 'player' else k
            expected = 4+17*k if role in ['admin', 'scorer'] else 4+12*k if role == 'viewer' else 4+13*k+4*(role == 'player')
            ...
            assert owner_calls == (0 if role == 'viewer' else attempts)
            assert owner_rows == (0 if role == 'viewer' else attempts*(2*size if role in ['admin', 'scorer'] else 2))
            ...
            assert categories == {'session':1+k+attempts, 'membership':1+k+attempts, 'authorization_round':attempts}
...
paths=subprocess.check_output(['git','ls-files','backend/src'],text=True).splitlines()
paths += ['backend/tests/match_authorization_measurement.rs'] + [str(p) for p in Path('backend/tests/match_authorization_measurement').glob('*.rs')]
