...
export function MatchResults({ tournamentId, selectedPlayerId, tournamentSelector }: { tournamentId: string; selectedPlayerId?: string; tournamentSelector?: ReactNode }) {
  ...
  const ready = table.data !== undefined
  const matches = rounds.data.filter(r => r.scoring_format === 'singles_match_play')
  return <section className="page match-page">{tournamentSelector}{table.data ? <><header className="page-header"><h1>Matchpoeng</h1></header>
    ...
    {matches.map(r => <section key={`${user}:${tournamentId}:${r.id}`}>{ready && <h2>{r.name}</h2>}<MatchRound roundId={r.id} playerId={playerId} ready={ready} /></section>)}
  </section>
}
