import { decodeNativeValue, validateNativeRanks } from './values'
...
function roundEntry(value: unknown, path: string): RoundLeaderboardEntry {
  ...
  return {
    ...
    playing_handicap: data.playing_handicap === null ? null : decodeInteger(data.playing_handicap, `${path}.playing_handicap`, undefined, undefined, 'resultatdata'),
...
export function decodeRoundLeaderboard(value: unknown, expectedRoundId: string, expectedTournamentId: string, expectedMetric: LeaderboardMetric): RoundLeaderboard {
  ...
  decoded.entries.forEach((entry, entryIndex) => {
    ...
    if ((decoded.scoring_format === 'four_ball_stroke_play') !== (entry.playing_handicap === null)) invalidLeaderboard(`${path}.playing_handicap`)
    const expectedType = ownerTypeForScoringFormat(decoded.scoring_format)
    ...
    if ((entry.value !== undefined) !== (decoded.scoring_format === 'individual_stableford')) invalidLeaderboard(`${path}.value`)
    if (entry.value && entry.value.actual_gross_total !== null
      && entry.value.actual_net_total !== entry.value.actual_gross_total - (entry.playing_handicap ?? invalidLeaderboard(`${path}.playing_handicap`))) invalidLeaderboard(`${path}.actual_net_total`)
  ...
  if (decoded.scoring_format === 'individual_stableford') validateNativeRanks(decoded)
  return decoded
}
