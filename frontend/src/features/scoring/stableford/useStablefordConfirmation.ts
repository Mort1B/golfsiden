...
interface ConfirmationInput {
  ...
  tournamentId: string
  owner: ScoreOwner
  card: StablefordScoringCard
  csrfToken: string | null
  onConfirmed: () => void
  onTerminal: () => void
}
export function useStablefordConfirmation(input: ConfirmationInput) {
    ...
    mutationFn: async () => {
      ...
      try {
        ...
        await scoreRequest(async signal => {
          const fresh = await stablefordApi.scoring(input.round.id, input.owner.id, signal)
          ...
          const card = await stablefordApi.confirm(input.round.id, input.owner.id, input.csrfToken ?? '', signal)
          if (!own() || signal.aborted) return
