...
test('profile edits, preserved trip handicap, duplicate/stale failures and all-device password logout', async ({ page, browser }) => {
  ...
  try {
    ...
    await page.getByLabel('Begrunnelse for handicapendring').fill('Oppdatert offisielt handicap')
    ...
    const changed = await secondPage.request.put('/api/me/profile', { headers: { 'x-csrf-token': otherSession.csrf_token }, data: {
      version: old.version, player_updated_at: old.player_updated_at, display_name: 'Navn fra en annen enhet', handicap: old.handicap, reason: '',
    } })
    ...
    const passwordForm = page.locator('form').filter({ has: page.getByRole('heading', { name: 'Bytt passord', exact: true }) })
