...
test('failed score remains guarded through delayed, failed and successful recovery', async ({ page }) => {
  const live = await liveServer()
  try {
    const state = await mockWorkspace(page, live.url)
    await page.goto(scoreUrl)
    await page.getByRole('button', { name: /Registrer par/ }).click()
    await expect(page.getByText('Lagring utilgjengelig')).toBeVisible()
    await stopped(live, page)
    await expect(page.getByText('Lagring utilgjengelig')).toBeVisible()
    let release: () => void = () => undefined
    state.pending = new Promise<void>(resolve => { release = resolve })
    live.resume(); await restored(page)
    await expect(page.getByText('Oppdaterer scoretilgang og rundestatus. Pågående scoreendring beholdes.')).toBeVisible()
    await layout(page, 'refreshing-failed-score')
