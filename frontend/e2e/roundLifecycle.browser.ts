...
test('admin lifecycle, live readiness, corrections, permissions and final visibility', async ({ page, browser }) => {
  ...
  await page.getByRole('link', { name: 'Rundestyring', exact: true }).click()
  ...
  for (const index of [0, 1, 2, 4]) {
    ...
    const confirmLink = page.locator('.round-lifecycle').getByRole('link', { name: `Bekreft scorekort for ${firstOwner.owner_name}`, exact: true })
