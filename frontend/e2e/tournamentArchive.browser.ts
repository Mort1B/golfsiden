...
test.skip(process.env.GOLF_TOURNAMENT_ARCHIVE_BROWSER !== '1', 'Requires fresh disposable seed and GOLF_TOURNAMENT_ARCHIVE_BROWSER=1.')
...
async function layout(page: Page, state: string) {
  for (const width of [320, 390, 1280]) {
    ...
    const panel = page.locator('.tournament-archive')
    ...
    await panel.screenshot({ path: `/tmp/golf-tournament-archive-${state}-${width}.png` })
  }
}

test('archive confirmation, live transition and retained private history', async ({ page, browser }) => {
  ...
  const panel = page.locator('.tournament-archive')
  const action = panel.getByRole('button', { name: 'Arkiver turneringen', exact: true })
  await page.goto('/tournaments')
  await expect(page.getByRole('link', { name: 'Nåværende (1)' })).toHaveAttribute('aria-current', 'page')
  await expect(page.getByRole('heading', { name: trip.name })).toBeVisible()
  await page.goto(url)
  await expect(action).toBeDisabled()
  await expect(panel.getByText('Fullfør turneringen før den arkiveres.')).toBeVisible()
  ...
  const current = decodeObject(await (await api.get(`/api/tournaments/${trip.id}`)).json(), 'trip')
  await mutate(api, `/api/tournaments/${trip.id}/complete`, auth.csrf_token, { expected_tournament_updated_at: current.updated_at })
  ...
  const secondPanel = secondPage.locator('.tournament-archive')
  await secondPanel.getByRole('button', { name: 'Arkiver turneringen', exact: true }).click()
  await secondPanel.getByRole('button', { name: 'Bekreft og arkiver turneringen' }).click()
  await expect(secondPanel.getByRole('heading', { name: 'Turneringen er arkivert' })).toBeVisible()
  await expect(panel.getByRole('heading', { name: 'Turneringen er arkivert' })).toBeVisible()
  await expect(panel.getByRole('group')).toHaveCount(0)
  await expect(action).toHaveCount(0)
  await layout(page, 'archived')
  ...
  await panel.getByRole('link', { name: 'Se turneringsarkivet' }).click()
  await expect(page).toHaveURL(/view=archived/)
  await expect(page.getByRole('heading', { name: trip.name })).toBeVisible()
  await page.getByRole('link', { name: 'Nåværende (0)' }).click()
  await expect(page.getByText('Ingen nåværende turneringer. Tidligere turneringer finnes i Arkiv.')).toBeVisible()
  await page.getByRole('link', { name: 'Alle (1)' }).click()
  await expect(page.getByRole('heading', { name: trip.name })).toBeVisible()
  await page.goBack()
  await expect(page.getByRole('link', { name: 'Nåværende (0)' })).toHaveAttribute('aria-current', 'page')
  await page.goBack()
  await expect(page.getByRole('link', { name: 'Arkiv (1)' })).toHaveAttribute('aria-current', 'page')
  await listLayout(page, 'archive')
  const member = await browser.newContext()
  const memberPage = await member.newPage()
  await login(memberPage, 'anders')
  await memberPage.goto('/tournaments?view=archived')
  await expect(memberPage.getByRole('heading', { name: trip.name })).toBeVisible()
  await memberPage.getByRole('link', { name: new RegExp(trip.name) }).click()
  await expect(memberPage).toHaveURL(`/tournaments/${trip.id}`)
  ...
  await expect(memberPage.locator('.tournament-archive')).toHaveCount(0)
  expect(errors).toEqual([])
  expect(failed).toEqual([])

  // Injected edge states follow the real authenticated workflow.
  await page.route(`**/api/tournaments/${trip.id}`, (route) => route.fulfill({ json: { ...trip, status: 'completed', name: 'LangtTurneringsnavnUtenMellomrom'.repeat(8) } }))
  await page.goto(url)
  await expect(action).toBeEnabled()
  await action.click()
  await layout(page, 'long')
  let finish: (() => void) | undefined
  await page.route(`**/api/tournaments/${trip.id}/archive`, async (route) => {
    await new Promise<void>((resolve) => { finish = resolve })
    await route.fulfill({ status: 409, json: { error: { code: 'tournament_archive_stale', message: 'Changed' } } })
  })
  await panel.getByRole('button', { name: 'Bekreft og arkiver turneringen' }).click()
  ...
  await page.route('**/api/me/tournaments', async (route) => {
    ...
    await route.fulfill({ json: mode === 'long' ? [{ tournament: { ...trip, status: 'archived', name: 'LangtTurneringsnavnUtenMellomrom'.repeat(8) }, role: 'admin', player_id: null }] : [] })
  })
  await page.goto('/tournaments?view=archived')
  await expect.poll(() => Boolean(release)).toBe(true)
  await expect(page.getByRole('status')).toBeVisible()
  await listLayout(page, 'loading')
  mode = 'empty'
  release?.()
  await expect(page.getByText('Du er ikke med i noen turneringer ennå.')).toBeVisible()
  await listLayout(page, 'empty')
  mode = 'long'
  await page.getByRole('button', { name: 'Oppdater turneringer' }).click()
  await expect(page.locator('.list-card')).toHaveCount(1)
  await listLayout(page, 'long')
  mode = 'error'
  await page.getByRole('button', { name: 'Oppdater turneringer' }).click()
  await expect(page.getByRole('alert')).toBeVisible({ timeout: 20000 })
  await expect(page.locator('.list-card')).toHaveCount(0)
  await listLayout(page, 'error')
  mode = 'empty'
...
async function listLayout(page: Page, state: string) {
  for (const width of [320, 390, 1280]) {
    await page.setViewportSize({ width, height: 900 })
    expect(await page.evaluate(() => document.documentElement.scrollWidth)).toBeLessThanOrEqual(width)
    for (const link of await page.getByRole('navigation', { name: 'Turneringsvisning' }).getByRole('link').all()) {
      expect((await link.boundingBox())?.height).toBeGreaterThanOrEqual(44)
      await link.click({ trial: true })
    }
    await page.screenshot({ path: `/tmp/golf-archive-list-${state}-${width}.png`, fullPage: true })
  }
}
