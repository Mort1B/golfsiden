...
VALUES ('20000000-0000-0000-0000-000000000005', '20000000-0000-0000-0000-000000000004', '20000000-0000-0000-0000-000000000001', 'Group 1');
INSERT INTO team_memberships (team_id, round_id, tournament_id, player_id, display_order) VALUES
('20000000-0000-0000-0000-000000000005', '20000000-0000-0000-0000-000000000004', '20000000-0000-0000-0000-000000000001', '20000000-0000-0000-0000-000000000011', 1),
('20000000-0000-0000-0000-000000000005', '20000000-0000-0000-0000-000000000004', '20000000-0000-0000-0000-000000000001', '20000000-0000-0000-0000-000000000012', 2),
('20000000-0000-0000-0000-000000000005', '20000000-0000-0000-0000-000000000004', '20000000-0000-0000-0000-000000000001', '20000000-0000-0000-0000-000000000013', 3);
"#;
...
async fn readiness_reports_pairing_eligibility_and_format_rules(pool: PgPool) {
    seed_ready(&pool).await;
    sqlx::query("DELETE FROM team_memberships WHERE player_id = $1")
        .bind(PLAYER_A)
        .execute(&pool)
        .await
        .unwrap();
    ...
    assert!(codes.contains(&ReadinessIssueCode::MissingTeamAssignment));
    assert!(codes.contains(&ReadinessIssueCode::IneligibleTeamAssignment));
    assert_eq!(validation.missing_players[0].player_id, PLAYER_A);
    assert_eq!(validation.ineligible_players.len(), 2);
...
async fn opened_round_freezes_configuration_pairings_and_snapshots_but_can_cascade(pool: PgPool) {
    ...
    let guarded_commands = [
        ...
        "DELETE FROM team_memberships WHERE team_id = '20000000-0000-0000-0000-000000000005' AND player_id = '20000000-0000-0000-0000-000000000011'",
        "DELETE FROM teams WHERE id = '20000000-0000-0000-0000-000000000005'",
...
#[sqlx::test(migrations = "../migrations")]
async fn assignment_and_removal_are_serialized_against_opening(pool: PgPool) {
    seed_ready(&pool).await;
    sqlx::query("DELETE FROM team_memberships WHERE player_id = $1")
        .bind(PLAYER_A)
        .execute(&pool)
