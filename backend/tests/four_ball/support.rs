...
pub const TOKEN: &str = "four-ball-admin-test-token";
pub async fn draft(pool: &PgPool, format: &str) -> Uuid {
    ...
    .await
    .unwrap();
    sqlx::query("INSERT INTO tournaments(id,name,start_date,end_date,number_of_rounds) VALUES($1,'Four-ball cup','2026-09-13','2026-09-13',1)").bind(id(2)).execute(pool).await.unwrap();
    sqlx::query(
    ...
    sqlx::query("INSERT INTO rounds(id,tournament_id,round_number,name,round_date,course_id,course_name,tee_id,tee_name,number_of_holes,scoring_format,handicap_allowance_percent) VALUES($1,$2,1,'Pairs','2026-09-13',$3,'Links',$4,'White',18,$5::text::scoring_format,85)").bind(id(5)).bind(id(2)).bind(id(3)).bind(id(4)).bind(format).execute(pool).await.unwrap();
    ...
    for (team, flight, players) in [(21, 31, [11, 12]), (22, 32, [13, 14])] {
        ...
        for player in players {
            sqlx::query("INSERT INTO team_memberships(team_id,round_id,tournament_id,player_id) VALUES($1,$2,$3,$4)").bind(id(team)).bind(id(5)).bind(id(2)).bind(id(player)).execute(pool).await.unwrap();
...
pub async fn ready(pool: &PgPool) -> Uuid {
    let session = draft(pool, "four_ball_stroke_play").await;
