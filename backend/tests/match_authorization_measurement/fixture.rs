// Synthetic fixture derived from stableford/support.rs; no production data.
...
pub async fn setup(pool: &PgPool, matches: u128) -> Uuid {
    let format = "singles_match_play";
    let count = 1i16;
    sqlx::query(
    ...
    .await
    .unwrap();
    sqlx::query("INSERT INTO tournaments(id,name,start_date,end_date,number_of_rounds,counted_rounds) VALUES($1,'Synthetic authorization measurement','2026-09-13','2026-09-13',$2,$3)").bind(id(2)).bind(count).bind(if format == "singles_match_play" { None } else { Some(1i16) }).execute(pool).await.unwrap();
    sqlx::query(
    ...
    sqlx::query("INSERT INTO rounds(id,tournament_id,round_number,name,round_date,course_id,course_name,tee_id,tee_name,number_of_holes,scoring_format,handicap_allowance_percent) VALUES($1,$2,1,'Synthetic match round','2026-09-13',$3,'Links',$4,'White',18,$5::text::scoring_format,100)").bind(id(5)).bind(id(2)).bind(id(3)).bind(id(4)).bind(format).execute(pool).await.unwrap();
    for index in 0..matches * 2 {
        let p = 1000 + index;
        let hcp = (index % 20) as i16;
    ...
    for index in 0..matches {
        let flight = 10000 + index;
        let players = [1000 + index * 2, 1001 + index * 2];
    ...
    let session = auth::create_session(
        pool,
        id(1),
        &hash_session_token("synthetic-measurement-admin"),
