...
async fn numeric_pickup_restore_preserves_identity_detects_aba_and_replays_receipt(pool: PgPool) {
    ...
    let replay = stableford::save_conditional(&pool, retry).await.unwrap();
    assert!(!replay.changed);
    assert_eq!(replay.value, first);
    let audit: Vec<(i64, Option<i16>)> = sqlx::query_as(
        "SELECT revision,gross_strokes FROM stableford_input_audits ORDER BY revision",
    )
    .fetch_all(&pool)
    .await
    .unwrap();
...
#[sqlx::test(migrations = "../migrations")]
async fn player_inputs_and_authority_reject_other_flight_viewer_and_legacy_paths(pool: PgPool) {
    ...
    assert!(
        stableford::get(&pool, viewer, id(5), id(11), true)
            .await
            .is_err()
    );
    ...
    for scoring in [false, true] {
        assert!(matches!(
            stableford::get(&pool, viewer, id(5), id(11), scoring).await,
            Err(ScorecardError::Forbidden)
        ));
    }
...
async fn private_read_is_actor_free_and_scoring_retains_revision_and_strict_api(pool: PgPool) {
    ...
    let path = format!("/api/rounds/{}/stableford/scorecards/{}", id(5), id(11));
    let read = request(&app, path.clone(), TOKEN).await;
    assert_eq!(read["holes"][0]["score"].as_object().unwrap().len(), 2);
    assert!(read.get("confirmed_by").is_none());
    let scoring = request(&app, format!("{path}/scoring"), TOKEN).await;
    assert_eq!(scoring["holes"][0]["score"]["revision"], "1");
    for input in [
        ...
        let response = app
            ...
            .oneshot(
                Request::put(format!(
                    "/api/rounds/{}/stableford/inputs/conditional",
                    id(5)
                ))
                .header("cookie", format!("golf_session={TOKEN}"))
...
async fn sql_boundary_rejects_bypass_identity_revision_and_history_mutations(pool: PgPool) {
    ...
    numeric(&pool, session, 11, 1, 4).await;
    for query in [
        "UPDATE stableford_inputs SET gross_strokes=5",
        "DELETE FROM stableford_inputs",
        "UPDATE stableford_input_audits SET gross_strokes=5",
        "DELETE FROM stableford_input_audits",
        "UPDATE stableford_mutation_receipts SET applied_revision=2",
        "DELETE FROM stableford_mutation_receipts",
    ] {
        assert!(sqlx::query(query).execute(&pool).await.is_err(), "{query}");
    }
    for query in [
        "UPDATE stableford_inputs SET revision=2",
        "UPDATE stableford_inputs SET id=gen_random_uuid()",
    ] {
        let mut tx = pool.begin().await.unwrap();
        sqlx::query("SELECT set_config('app.score_mutation_round_id',$1::text,true),set_config('app.stableford_actor_id',$2::text,true),set_config('app.stableford_session_id',$3::text,true)").bind(id(5)).bind(id(1)).bind(session).execute(&mut *tx).await.unwrap();
        assert!(sqlx::query(query).execute(&mut *tx).await.is_err());
    }
    assert_eq!(
        sqlx::query_scalar::<_, i64>("SELECT revision FROM stableford_inputs")
            .fetch_one(&pool)
            .await
            .unwrap(),
        1
...
async fn ancestor_deletion_cascades_inputs_audits_and_receipts(pool: PgPool) {
    ...
    sqlx::query("DELETE FROM tournaments WHERE id=$1")
        ...
        .unwrap();
    for query in [
        "SELECT count(*) FROM stableford_inputs",
        "SELECT count(*) FROM stableford_input_audits",
        "SELECT count(*) FROM stableford_mutation_receipts",
    ] {
        assert_eq!(
...
async fn private_get_checks_membership_before_format_mismatch(pool: PgPool) {
    draft(&pool, "individual_stroke_play").await;
    ...
    for (round, token, expected) in [
        ...
        for suffix in ["", "/scoring"] {
            let response = app
                ...
                .oneshot(
                    Request::get(format!(
                        "/api/rounds/{round}/stableford/scorecards/{}{suffix}",
                        id(11)
                    ))
                    .header("cookie", format!("golf_session={token}"))
                    .body(Body::empty())
