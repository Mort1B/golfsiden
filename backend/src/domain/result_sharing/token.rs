...
pub struct ResultShareToken(String);
pub struct ResultShareTokenHash([u8; 32]);

impl ResultShareToken {
    ...
    pub fn hash(&self) -> ResultShareTokenHash {
        ResultShareTokenHash(Sha256::digest(self.0.as_bytes()).into())
    }
...
mod tests {
    ...
    fn tokens_are_independent_256_bit_secrets_with_exact_hashes() {
        let a = ResultShareToken::generate().unwrap();
        let b = ResultShareToken::generate().unwrap();
        assert_ne!(a.expose(), b.expose());
        assert_eq!(a.expose().len(), 43);
        assert!(a.matches(a.hash().as_bytes()));
        assert!(!a.matches(b.hash().as_bytes()));
        assert!(ResultShareToken::parse(a.expose().to_owned()).is_some());
        assert!(ResultShareToken::parse("invalid".into()).is_none());
    }
}
