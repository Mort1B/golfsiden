...
pub async fn save_conditional(
    ...
    super::player(&mut tx, input.round_id, player).await?;
    ...
    let receipt=sqlx::query_as::<_,Receipt>("SELECT request_hash,applied_score_id,applied_revision FROM stableford_mutation_receipts WHERE user_id=$1 AND request_id=$2").bind(actor).bind(input.request_id).fetch_optional(&mut *tx).await?;
    ...
    let current=sqlx::query_as::<_,InputRow>("SELECT id,revision,round_id,hole_id,player_id,gross_strokes,submitted_by,submitted_at,updated_at FROM stableford_inputs WHERE round_id=$1 AND hole_id=$2 AND player_id=$3 FOR UPDATE").bind(input.round_id).bind(input.hole_id).bind(player).fetch_optional(&mut *tx).await?;
    ...
    let (id, revision) = if changed {
        sqlx::query_as::<_,(Uuid,i64)>("INSERT INTO stableford_inputs(id,round_id,tournament_id,hole_id,player_id,gross_strokes,submitted_by) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT(round_id,hole_id,player_id) DO UPDATE SET gross_strokes=EXCLUDED.gross_strokes,submitted_by=EXCLUDED.submitted_by RETURNING id,revision")
    ...
    sqlx::query("SELECT set_config('app.stableford_request_id',$1::text,true)")
        .bind(input.request_id)
        .execute(&mut *tx)
        .await?;
    let inserted=sqlx::query("INSERT INTO stableford_mutation_receipts(user_id,request_id,round_id,request_hash,applied_score_id,applied_revision) VALUES($1,$2,$3,$4,$5,$6) ON CONFLICT(user_id,request_id) DO NOTHING").bind(actor).bind(input.request_id).bind(input.round_id).bind(hash).bind(id).bind(revision).execute(&mut *tx).await?.rows_affected()==1;
    if !inserted {
        return Err(ScorecardError::Conflict(ScorecardConflict::RequestMismatch));
    }
    live(&mut tx, input.session_id).await?;
...
pub async fn confirm(
    ...
    let actor = authorize_player(&mut tx, session_id, &context, player_id).await?;
    let mut card = build(&mut tx, &context, player, unrestricted()).await?;
    ...
    if changed {
        ...
        let confirmed_at=sqlx::query_scalar("INSERT INTO scorecard_confirmations(id,round_id,tournament_id,player_id,confirmed_by) VALUES($1,$2,$3,$4,$5) RETURNING confirmed_at").bind(Uuid::new_v4()).bind(round_id).bind(context.tournament_id).bind(player_id).bind(actor).fetch_one(&mut *tx).await?;
...
async fn set_context(
    ...
    sqlx::query("SELECT set_config('app.score_mutation_round_id',$1::text,true),set_config('app.stableford_session_id',$2::text,true),set_config('app.stableford_actor_id',$3::text,true)").bind(round).bind(session).bind(actor).execute(&mut **tx).await?;
    Ok(())
}
