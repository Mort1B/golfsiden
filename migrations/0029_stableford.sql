-- Dedicated Stableford inputs; legacy numeric data and receipts remain unchanged.
ALTER TYPE scoring_format ADD VALUE 'individual_stableford';
ALTER TABLE rounds ADD CONSTRAINT stableford_18_holes CHECK(scoring_format::text <> 'individual_stableford' OR number_of_holes=18);
CREATE TABLE stableford_inputs (
 id UUID PRIMARY KEY,
...
-- Player-card reads select the retained hole inputs for one player and round.
CREATE INDEX stableford_inputs_player_idx ON stableford_inputs(round_id,player_id,hole_id);
CREATE TABLE stableford_input_audits (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
 input_id UUID NOT NULL REFERENCES stableford_inputs(id) ON DELETE CASCADE,
...
CREATE FUNCTION stableford_player_authorized(target_round UUID,target_player UUID,actor UUID,session UUID) RETURNS BOOLEAN LANGUAGE sql STABLE AS $$
...
$$;
CREATE FUNCTION guard_stableford_input() RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
 ...
 IF TG_OP='DELETE' THEN RAISE EXCEPTION 'Stableford inputs retain their identity' USING ERRCODE='23514'; END IF;
 PERFORM acquire_score_round_lock(NEW.round_id);
 SELECT * INTO r FROM rounds WHERE id=NEW.round_id;
 actor=NULLIF(current_setting('app.stableford_actor_id',true),'')::UUID;
 session=NULLIF(current_setting('app.stableford_session_id',true),'')::UUID;
 IF r.scoring_format::text IS DISTINCT FROM 'individual_stableford' OR r.status NOT IN ('open','completed')
 OR current_setting('app.score_mutation_round_id',true) IS DISTINCT FROM NEW.round_id::text
 OR actor IS DISTINCT FROM NEW.submitted_by OR NOT stableford_player_authorized(NEW.round_id,NEW.player_id,actor,session)
 OR NOT EXISTS(SELECT 1 FROM holes WHERE id=NEW.hole_id AND tee_id=r.tee_id) THEN
 RAISE EXCEPTION 'invalid Stableford input context' USING ERRCODE='23514',CONSTRAINT='stableford_input_context'; END IF;
 ...
 RETURN NEW;
END $$;
CREATE TRIGGER stableford_inputs_guard BEFORE INSERT OR UPDATE OR DELETE ON stableford_inputs FOR EACH ROW EXECUTE FUNCTION guard_stableford_input();
CREATE FUNCTION audit_stableford_input() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
 IF TG_OP='INSERT' OR NEW.gross_strokes IS DISTINCT FROM OLD.gross_strokes THEN
 INSERT INTO stableford_input_audits(input_id,revision,previous_gross_strokes,gross_strokes,changed_by)
 ...
 DELETE FROM scorecard_confirmations WHERE round_id=NEW.round_id AND player_id=NEW.player_id;
 END IF;RETURN NEW;
END $$;
CREATE TRIGGER stableford_inputs_audit AFTER INSERT OR UPDATE ON stableford_inputs FOR EACH ROW EXECUTE FUNCTION audit_stableford_input();
CREATE FUNCTION guard_stableford_audit() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
 IF TG_OP='INSERT' AND pg_trigger_depth()>1 AND EXISTS(SELECT 1 FROM stableford_inputs i WHERE i.id=NEW.input_id AND i.revision=NEW.revision AND i.submitted_by=NEW.changed_by AND i.gross_strokes IS NOT DISTINCT FROM NEW.gross_strokes) THEN RETURN NEW; END IF;
 IF TG_OP='DELETE' AND NOT EXISTS(SELECT 1 FROM stableford_inputs WHERE id=OLD.input_id) THEN RETURN OLD; END IF;
 RAISE EXCEPTION 'Stableford input audits are append-only' USING ERRCODE='23514';
END $$;
CREATE TRIGGER stableford_audits_guard BEFORE INSERT OR UPDATE OR DELETE ON stableford_input_audits FOR EACH ROW EXECUTE FUNCTION guard_stableford_audit();
CREATE TABLE stableford_mutation_receipts (
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    request_id UUID NOT NULL,
    round_id UUID NOT NULL REFERENCES rounds(id) ON DELETE CASCADE,
    request_hash BYTEA NOT NULL CHECK(octet_length(request_hash)=32),
    applied_score_id UUID NOT NULL REFERENCES stableford_inputs(id) ON DELETE CASCADE,
    applied_revision BIGINT NOT NULL CHECK(applied_revision>0),
    applied_at TIMESTAMPTZ NOT NULL DEFAULT clock_timestamp(),
    PRIMARY KEY(user_id,request_id)
);
CREATE INDEX stableford_mutation_receipts_round_idx ON stableford_mutation_receipts(round_id);
CREATE INDEX stableford_mutation_receipts_score_idx ON stableford_mutation_receipts(applied_score_id);
CREATE FUNCTION guard_stableford_mutation_receipt() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    IF TG_OP='INSERT' THEN
        IF current_setting('app.stableford_actor_id',true) IS DISTINCT FROM NEW.user_id::TEXT
           OR current_setting('app.stableford_request_id',true) IS DISTINCT FROM NEW.request_id::TEXT
           OR current_setting('app.score_mutation_round_id',true) IS DISTINCT FROM NEW.round_id::TEXT
           OR NOT EXISTS(SELECT 1 FROM stableford_inputs WHERE id=NEW.applied_score_id AND round_id=NEW.round_id AND revision=NEW.applied_revision) THEN
    ...
    IF TG_OP='UPDATE' OR (
        ...
        AND EXISTS(SELECT 1 FROM stableford_inputs WHERE id=OLD.applied_score_id)
    ) THEN
...
END;
$$;
CREATE TRIGGER stableford_mutation_receipts_guard BEFORE INSERT OR UPDATE OR DELETE ON stableford_mutation_receipts
FOR EACH ROW EXECUTE FUNCTION guard_stableford_mutation_receipt();CREATE OR REPLACE FUNCTION validate_score_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
    ...
    IF parent_format IN ('four_ball_stroke_play','individual_stableford') THEN RAISE EXCEPTION 'four-ball uses dedicated inputs' USING ERRCODE='23514',CONSTRAINT='score_owner_format_mismatch'; END IF;
    ...
    END IF;
    IF (parent_format IN ('individual_stroke_play','individual_stableford') AND (NEW.player_id IS NULL OR NEW.team_id IS NOT NULL))
...
BEGIN
    ...
    IF parent_status NOT IN ('open', 'completed')
       AND NOT (parent_status = 'locked' AND current_setting('app.admin_correction', true) = 'true') THEN
        RAISE EXCEPTION 'round is not open for scorecard confirmation'
            USING ERRCODE = '23514', CONSTRAINT = 'score_round_not_editable';
    END IF;
    IF (parent_format IN ('individual_stroke_play','individual_stableford') AND (target_player_id IS NULL OR target_team_id IS NOT NULL))
    ...
    IF parent_format = 'two_player_foursomes' AND NOT EXISTS (
        ...
        RAISE EXCEPTION 'team was not snapshotted for this round'
            USING ERRCODE = '23514', CONSTRAINT = 'score_owner_ineligible';
    END IF;
    IF parent_format='individual_stableford' THEN
      IF parent_status NOT IN ('open','completed') OR NOT stableford_player_authorized(target_round_id,target_player_id,NEW.confirmed_by,NULLIF(current_setting('app.stableford_session_id',true),'')::UUID) THEN
        RAISE EXCEPTION 'Stableford confirmation requires player authority' USING ERRCODE='23514'; END IF;
      SELECT count(*) INTO scored_holes FROM stableford_inputs WHERE round_id=target_round_id AND player_id=target_player_id;
    ELSIF parent_format='four_ball_stroke_play' THEN
...
BEGIN
    ...
    IF target_format = 'individual_stableford' THEN
        SELECT count(*) INTO owner_count FROM round_handicap_snapshots WHERE round_id=target_round_id;
        SELECT count(*) INTO invalid_owner_count FROM round_handicap_snapshots rhs WHERE rhs.round_id=target_round_id AND
          (required_holes<>(SELECT count(*) FROM stableford_inputs i WHERE i.round_id=target_round_id AND i.player_id=rhs.player_id) OR NOT EXISTS(SELECT 1 FROM scorecard_confirmations c WHERE c.round_id=target_round_id AND c.player_id=rhs.player_id));
    ELSIF target_format = 'individual_stroke_play' THEN
        SELECT count(*) INTO owner_count FROM round_handicap_snapshots WHERE round_id = target_round_id;
...
-- Opening owns the snapshot transaction; individual Stableford requires every entrant in a flight, never a team.
CREATE FUNCTION guard_stableford_opening() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
 IF OLD.status='draft' AND NEW.status='open' AND NEW.scoring_format::text='individual_stableford' THEN
   IF NEW.number_of_holes<>18 OR NOT EXISTS(SELECT 1 FROM round_handicap_snapshots WHERE round_id=NEW.id)
   OR EXISTS(SELECT 1 FROM teams WHERE round_id=NEW.id)
   OR EXISTS(SELECT 1 FROM round_handicap_snapshots s WHERE s.round_id=NEW.id AND NOT EXISTS(SELECT 1 FROM flight_memberships f WHERE f.round_id=s.round_id AND f.player_id=s.player_id))
   OR EXISTS(SELECT 1 FROM flight_memberships f WHERE f.round_id=NEW.id AND NOT EXISTS(SELECT 1 FROM round_handicap_snapshots s WHERE s.round_id=f.round_id AND s.player_id=f.player_id))
   THEN RAISE EXCEPTION 'Stableford opening requires eligible player flights and 18 holes' USING ERRCODE='23514'; END IF;
 END IF;
 RETURN NEW;
END $$;
CREATE TRIGGER stableford_opening_guard BEFORE UPDATE OF status ON rounds FOR EACH ROW EXECUTE FUNCTION guard_stableford_opening();
